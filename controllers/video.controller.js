...
const VideoController = {
    ...
    addSubtitles : asyncHandler(async (req, res) => {
      ...
      if (!id || !text || startTime === undefined || endTime === undefined) {
        const error = new Error('Missing video ID, subtitle text, startTime, or endTime.');
        error.statusCode = 400;
        throw error;
      }
    
      if (endTime <= startTime) {
        const error = new Error('Subtitle end time must be greater than start time.');
        error.statusCode = 400;
        throw error;
      }
      ...
      try {
        const subtitle = await videoService.addSubtitles(id, text, startTime, endTime);
        res.json(subtitle);
      } catch (error) {
    ...
    renderVideo : asyncHandler(async (req, res) => {
      ...
      try {
        const result = await videoService.renderVideo(id);
        res.json({ message: 'Rendered successfully', result });
      } catch (error) {
        console.error('Error in videoService.renderVideo:', error);
        const serviceError = new Error('Failed to render the video.');
        serviceError.statusCode = 500;
        throw serviceError;
      }
    }),
